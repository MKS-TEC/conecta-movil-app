...
import 'package:conecta/presentation/initial_configuration/care_program.dart';
...
class _PetInformationState extends State<PetInformation> {
  ...
  void _onSearchBreeds(String value) {
    if (value.length > 0) {
      ...
      breeds.forEach((Breed breed) {
        if (breed.name!.trim().toLowerCase().startsWith(value.toLowerCase())) {
          print(breed.name!);
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getIt<InitialConfigurationBloc>()..add(GetBreedsBySpecies(widget.pet.species ?? '')),
