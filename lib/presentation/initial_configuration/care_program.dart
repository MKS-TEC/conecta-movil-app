...
class _CareProgramState extends State<CareProgram> {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider<InitialConfigurationBloc>(
          create: (BuildContext context) => getIt<InitialConfigurationBloc>(),
        ),
      ], 
        ...
        BlocListener<AuthBloc, AuthState>(
          listener: (context, state) {
            if (state is AuthenticatedSubjectLoaded) {
              ...
              context.read<InitialConfigurationBloc>().add(GetPrograms(widget.pet));
            }
          },
        ),
        BlocListener<InitialConfigurationBloc, InitialConfigurationState>(
          listener: (context, state) {
            if (state is SuccessfulSetPet){
              ...
              context.read<InitialConfigurationBloc>().add(CreatePrograms(state.pet.petId!, programsPet));
            }
            ...
            if (state is SuccessfulCreatePrograms) {
              context.read<InitialConfigurationBloc>().add(CreateCalendar(petId!, calendarEvents));
            }

            if (state is SuccessfulCreatePrograms) {
               if (_isMedicHistorial) {
                Navigator.of(context).pushAndRemoveUntil(
                  MaterialPageRoute(builder: (context) => Dashboard(viewInit: 2)), 
  ...
  Widget _careProgramsErrorWidget(BuildContext context) {
    return BlocBuilder<InitialConfigurationBloc, InitialConfigurationState>(
                ...
                children: <Widget>[
                  Expanded(
                    child: ButtonSecondary(
                      ...
                      onPressed: () {
                        setState(() {
                          _isMedicHistorial = true;
                        });

                        context.read<InitialConfigurationBloc>()
